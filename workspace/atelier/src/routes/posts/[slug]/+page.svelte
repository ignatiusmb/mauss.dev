...
<Article post={data.article} path={data.source} flank={data.article.flank}>
	...
	{#if data.article.title.startsWith('Accessibility!')}
		<section data-info="important">
			...
			<!-- prettier-ignore -->
			<p>Many of these widgets were later incorporated into HTML5, and <strong>developers should prefer using the correct semantic HTML element over using ARIA,</strong> if such an element exists. For instance, native elements have built-in <a href="https://developer.mozilla.org/en-US/docs/Web/Accessibility/Keyboard-navigable_JavaScript_widgets" target="_blank">keyboard accessibility</a>, roles and states. However, if you choose to use ARIA, you are responsible for mimicking (the equivalent) browser behavior in script. Source: <a href="https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA" target="_blank">MDN</a></p>

			<hr style:background="oklch(0.783 0.16 88)" style:margin="1rem 0" />
